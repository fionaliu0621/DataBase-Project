...
function RegisterForm({ onSuccess }) {
  ...
  const [done,      setDone]      = useState(null); // 存放成功後的 customer_id
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ firstName, lastName, city, zipCode: zip })
      });
    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
        <div>
          ...
          <input value={firstName} onChange={e => setFirstName(e.target.value)} style={inputLight} />
        </div>
        <div>
          <div style={fieldLabel}>LAST NAME</div>
          <input value={lastName} onChange={e => setLastName(e.target.value)} style={inputLight} />
        </div>
      </div>

      <div style={{ marginBottom: 14 }}>
        <div style={fieldLabel}>CITY（選填）</div>
        <input value={city} onChange={e => setCity(e.target.value)} placeholder="e.g. Taipei" style={inputLight} />
      </div>

      <div style={{ marginBottom: 24 }}>
        <div style={fieldLabel}>ZIP CODE（選填）</div>
        <input value={zip} onChange={e => setZip(e.target.value)} placeholder="e.g. 100" style={inputLight} />
      </div>

      {error && (
        <div style={{ fontSize: 11, color: "#e24b4a", marginBottom: "1rem", lineHeight: 1.6 }}>{error}</div>
      )}
      ...
      <div style={{ fontSize: 10, color: "#bbb", textAlign: "center", lineHeight: 1.7, letterSpacing: "0.2px" }}>
        註冊後系統會自動產生你的 Customer ID，請記下來用於下次登入
      </div>
    </>
  );
}
...
export default function LoginPage() {
  ...
  const handleSignIn = () => {
    if (!customerId.trim()) {
      setSignInError("請輸入 Customer ID（例如 cust_000001）");
      return;
    }
    setSignInError(null);
